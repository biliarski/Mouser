...
from ctypes import c_long, POINTER, sizeof
...
def main():
    ...
    while True:
        ...
        if t - last_click > 0.3:
            ...
            inp.mi.dwFlags = MOUSEEVENTF_MOVE  # | MOUSEEVENTF_LEFTDOWN no clicking
            res = send_input_func(1, ctypes.pointer(inp), sizeof(INPUT))
            if res != 1:
                ctypes.FormatError(ctypes.GetLastError())

            # Send mouse up input event
            inp.mi.dwFlags = MOUSEEVENTF_MOVE  # | MOUSEEVENTF_LEFTUP no clicking
            res = send_input_func(1, ctypes.pointer(inp), sizeof(INPUT))
            if res != 1:
                ctypes.FormatError(ctypes.GetLastError())

        try:  # used try so that if user pressed other than the given key error will not be shown
            if keyboard.is_pressed('q') or keyboard.is_pressed('esc'):  # if key 'q' or 'escape' is pressed
                print('Exiting Mouser.')
